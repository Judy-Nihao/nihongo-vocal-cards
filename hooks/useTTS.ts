...
export const useTTS = () => {
  ...
  const getVoicesSafe = useCallback(() => {
    ...
    const jaBrowserVoices = allBrowserVoices.filter(
  ...
  useEffect(() => {
    const loadVoices = () => {
      ...
      if (!selectedVoice && combinedVoices.length > 0) {
        ...
        // Validate existence
        const exists = combinedVoices.find(
  ...
  const playBrowserTTS = (text: string, voiceName: string) => {
    ...
    let targetVoice = currentVoices.find(
      (v) => `[Device] ${v.name}`.includes(voiceName) || v.name === voiceName,
    );
  ...
  const playGeminiTTS = async (text: string, voiceNameRaw: string) => {
    ...
    try {
      const voiceNameKey =
        GEMINI_VOICES.find((v) => voiceNameRaw.includes(v.name))?.name ||
        "Kore";
      ...
      if (error instanceof Error && error.message?.startsWith("429:")) {
        alert(TEXT.ERRORS.API_QUOTA_SIMPLE);
      } else {
